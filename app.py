...
def _render_filters(df: pd.DataFrame):
    ...
    if "Appointment" in df.columns:
        ...
        if not datum_vals.empty:
            ...
            # Standaard: vorige werkweek. Maar als die volledig buiten de
            # beschikbare data valt (bijv. data loopt achter op de kalender),
            # toon dan het hele beschikbare bereik i.p.v. één losse randdag.
            if vw_van > max_d or vw_tot < min_d:
                default_van, default_tot = min_d, max_d
            else:
